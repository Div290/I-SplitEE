...
def main(args):
	...
	model_save_path = os.path.join(config.DIR_NAME, 'AdaEE', "models", "ee_model_pristine_cifar.pth")
	...
	history_path = '/home/divya/Aastha_early_exits_work/datasets/history_ee_model_pristine_cifar.csv'
	...
	distortion_values = config.distortion_lvl_dict["pristine"]
	train_loader, val_loader, test_loader = utils.load_cifar10(args, dataset_path, indices_path, distortion_values)
	...
	ee_model = ee_nn.Early_Exit_DNN(args.model_name, n_classes, args.pretrained, args.n_branches, 300, device, args.exit_type, args.distribution)
	...
	loss_weights = [1, 1, 1, 0.7, 0.7, 0.7, 0.5, 0.5, 0.5, 0.5]#, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5]
	...
	while (count < 50):
		...
		val_result = utils.eval_EEDNNs(ee_model, val_loader, criterion, n_exits, epoch, device, loss_weights)
		# print(type(train_result))
		scheduler.step()
		current_result.update(train_result), current_result.update(val_result)
		# print(current_result)
		current_res = pd.Series(current_result)
		df_new = df_new._append(current_res, ignore_index=True)
		df_new.to_csv(history_path)
...
if (__name__ == "__main__"):
	...
	parser.add_argument('--distortion_type', type=str, default=config.distortion_type, 
		choices=["caltech256", "cifar10"], help='Distortion Type.') 
    ...
    # parser.add_argument('--n_rounds', type=int, default=config.n_rounds, help='Epochs.')
	...
	# parser = argparse.ArgumentParser(description="Testing Early-exit DNN")
	# parser.add_argument('--dataset_name', type=str, default=config.dataset_name, choices=["caltech256", "cifar10"],
                        # help='Dataset name.')
	# parser.add_argument('--cuda', type=bool, default=config.cuda, help='Cuda? Default: %s' % (config.cuda))
